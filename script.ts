...
var sizeInput = document.getElementById("size") as HTMLInputElement;
...
class Cell {
    ...
    x: number;
    y: number;
    constructor(public row: number, public coll: number) {
        this.value = 0;
...
function moveUp() {
    let change = false;
    for (let j = 0; j < size; j++) { //for every column do
        for (let i = 0; i < size; i++) { //start from top row of column and go down
            if (cells[i][j].value) {
                let row = i;
                while (row > 0) {
